...
class Mage_Catalog_Model_Layer_Filter_Attribute extends Mage_Catalog_Model_Layer_Filter_Abstract
{
    ...
    protected function _getItemsData()
    {
        ...
        if ($data === null) {
            ...
            $data = array();
            /*--*/
            $array_count_sum =  array_sum($optionsCount);
            $array_new_sum = 0;
            /*--*/
            foreach ($options as $option) {
                ...
                if (Mage::helper('core/string')->strlen($option['value'])) {
                    ...
                    /*--*/
                        if(trim($this->getName()) == trim("Customer Review")) {
                            $filter_count = isset($optionsCount[$option['value']]) ? $optionsCount[$option['value']] : 0;
                            $data[] = array(
                                'label' => $option['label'],
                                'value' => $option['value'],
                                'count' => $array_count_sum - $array_new_sum,
                            );                        
                            $array_new_sum += $filter_count;
